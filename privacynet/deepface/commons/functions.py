...
def extract_faces(
    ...
    # CHECK: For now if detection is not detected just return the whole image
    region_obj = {
        "x": 0,
        "y": 0,
        "w": target_size[0],
    ...
    # We might still need some padding.
    ...
    img_pixels /= 255  # CHECK: If division is necessary

    assert current_img.shape[0:2] == target_size, "Still non-matching sizes"

    return [(img_pixels, region_obj, 0)]


def extract_faces_involved(
    img: Union[str, np.ndarray],
    target_size: tuple = (224, 224),
    detector_backend: str = "opencv",
    grayscale: bool = False,
